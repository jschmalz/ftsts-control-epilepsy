...
for i = 1:duration/step
    ...
    % anodic phase
    if counter >= 0 && counter < T_stim + step*0
        Ue(i) = -V_stim/multi;
    end
    
    % cathodic phase
    if counter >= T_stim && counter < 2*T_stim  + step*0
        Ue(i) = V_stim;
    end
    
    if counter >= 2*T_stim  + step && counter < (2+x)*T_stim  + step*0
        Ue(i) = 0;
    end
...
for i = 1:duration/step
    ...
    % anodic phase
    if counter >= T_stim && counter < (multi+1)*T_stim + 0*step
        Ui(i) = -V_stim/multi;
    end
